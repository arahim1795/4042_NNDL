...
def cnn(images):
    ...
    W1 = tf.Variable(
        tf.truncated_normal(
            [9, 9, NUM_CHANNELS, 50], stddev=1.0 / np.sqrt(NUM_CHANNELS * 9 * 9)
        ),
        name="weights_1",
    )
    ...
    pool_1 = tf.nn.max_pool(
        conv_1, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding="VALID", name="pool_1"
    )

    dim = (
        pool_1.get_shape()[1].value
        * pool_1.get_shape()[2].value
        * pool_1.get_shape()[3].value
    )
    pool_1_flat = tf.reshape(pool_1, [-1, dim])

    # Softmax
    W2 = tf.Variable(
        tf.truncated_normal([dim, NUM_CLASSES], stddev=1.0 / np.sqrt(dim)),
        name="weights_3",
    )
