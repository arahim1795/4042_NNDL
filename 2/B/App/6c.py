...
def word_rnn_model_LSTM_2layer(train_data, test_data, keep_probability):
    ...
    # hidden layer
    cell = tf.nn.rnn_cell.MultiRNNCell(
        [tf.nn.rnn_cell.LSTMCell(HIDDEN_SIZE), tf.nn.rnn_cell.LSTMCell(HIDDEN_SIZE)]
    )
    _, encoding = tf.nn.static_rnn(cell, word_list, dtype=tf.float32)
    encoding = encoding[-1]
    encoding = encoding[-1]
...
def char_rnn_model_vanilla_2layer_gradient_clipping(
    train_data, test_data, keep_probability
):
    # Create the model
    x = tf.placeholder(tf.int64, [None, MAX_DOCUMENT_LENGTH])
    y_ = tf.placeholder(tf.int64)
    ...
    capped_gvs = [
        (tf.clip_by_value(grad, -2.0, 2.0), var) for grad, var in gvs
    ]  # +/- 2 for threshold =2
...
def word_rnn_model_vanilla_2layer_gradient_clipping(
    train_data, test_data, keep_probability
):
    # Create the model
    x = tf.placeholder(tf.int64, [None, MAX_DOCUMENT_LENGTH])
...
def char_rnn_model_LSTM_2layer_gradient_clipping(
    train_data, test_data, keep_probability
):
    # Create the model
    x = tf.placeholder(tf.int64, [None, MAX_DOCUMENT_LENGTH])
...
def word_rnn_model_LSTM_2layer_gradient_clipping(
    train_data, test_data, keep_probability
):
    # Create the model
    x = tf.placeholder(tf.int64, [None, MAX_DOCUMENT_LENGTH])
    ...
    gvs = train_op.compute_gradients(entropy)
    capped_gvs = [
        (tf.clip_by_value(gradient, -2.0, 2.0), var) for gradient, var in gvs
    ]  # +/- 2 for threshold =2
    train_op = train_op.apply_gradients(capped_gvs)
